  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div>
        ...
        <p className="text-xs sm:text-sm text-gray-600 mt-1">Record wholesale purchases (stock coming in)</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-lg border border-gray-200 bg-white p-4 sm:p-5 lg:p-6 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          ...
          <div>
            <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Total Cost (LKR) *</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Supplier (Optional)</label>
            <input
              type="text"
      ...
      <div>
        ...
        <div className="space-y-3">
          {purchaseStatus === 'loading' && <p className="text-center text-gray-500 py-8">Loading purchases...</p>}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-sm text-gray-500">{new Date(purchase.date).toLocaleDateString()}</span>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-900">LKR {purchase.totalCost.toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-gray-600">Cost/Item</p>
                        <p className="font-medium text-gray-900">LKR {purchase.costPerItem.toFixed(2)}</p>
                      </div>
                      <div>
                        <p className="text-gray-600">Selling Price</p>
                        <p className="font-medium text-green-600">LKR {purchase.sellingPricePerItem}</p>
                      </div>
                    </div>
                    {purchase.supplier && (
                      <p className="mt-2 text-sm text-gray-600">Supplier: {purchase.supplier}</p>
                    )}
                  </div>
                  <button
